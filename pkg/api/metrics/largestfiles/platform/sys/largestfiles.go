...
// LargestFiles represents an empty LargestFiles entity on the current system.
type LargestFiles struct{}

// List returns a list of LargestFiles stats
func (lf LargestFiles) List(top100files []metrics.PathSize) ([]rpi.LargestFiles, error) {
	var result []rpi.LargestFiles

	for i := range top100files {
		fileCat := FileCategory(top100files[i].Path)
		result = append(result, rpi.LargestFiles{
