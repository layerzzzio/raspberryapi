...
func (ls *LogService) ExecuteDPTOOL(ctx echo.Context, deployType string, url string, version string) (resp rpi.Action, err error) {
	defer func(begin time.Time) {
		ls.logger.Log(
			ctx,
			name,
			"request: "+deployType+" api version "+url+"/"+version,
			err,
	...
	return ls.Service.ExecuteDPTOOL(deployType, url, version)
}
